...
/datum/species/xenos/tyranid/genestealer
	...
	mob_size = MOB_HUGE //fat fuck
	strength = STR_VHIGH
	teeth_type = /obj/item/stack/teeth/genestealer
...
//	siemens_coefficient = 0 //reliant on further generations for hacking
...
/mob/living/carbon/human
	var/new_genestealer = SPECIES_GENESTEALER
	var/biomass = 100
	var/isconverting = 0
	var/dnastore = 0
	var/poolparty = 0
...
/datum/species/xenos/tyranid/genestealer/initialize()
	. = ..()
	...
	add_stats(rand(18,20),rand(16,24),18,16) //not lore accurate, but random STR and DEXT is good.
	thirst = INFINITY
	nutrition = INFINITY
	bladder = -INFINITY
	bowels = -INFINITY
...
/mob/living/carbon/human/genestealer/proc/givestealerstats()
	...
	if(src.stat == DEAD)
		to_chat(src, "<span class='notice'>You should have done that when you were alive.</span>")
		return

	visible_message("[name] listens intently to the will of the broodmind. Now is the time! The fleet is near!")
	...
	src.verbs -= /mob/living/carbon/human/genestealer/proc/givestealerstats //removes verb at the end so they can't spam it for better stats
*/

/datum/species/xenos/tyranid/genestealer/verb/convert()
	...
	H.confused++
	H.drowsyness++
	T.adjustOxyLoss(-1)  //almost a 100% aheal to their victim
	T.adjustBruteLoss(-100) //heals their brute injuries, stopping bleeding
	T.adjustFireLoss(-100) //heals their burn injuries
	T.adjustToxLoss(-100) //heals their organs but not their Internal Bleeding or broken bones.
	T.adjustBrainLoss(-100) //"wont this revive the dead?" no it wont, if their brain is DECAYED/ded they wont be revived, but if they have exactly 1% of health, they will live once more
	T.inject_blood(src, 560) //fully heals blood
	return 1

/mob/living/carbon/human/genestealer/proc/ripperswarm() // ok
	set name = "Call on 3 Rippers(10)"
	set desc = "Distract the enemy by invoking 3 rippers from underground, be aware, these rippers may attack your cultists or anyone who is agressive towards them."
	set category = "Tyranid"

	if(src.biomass < 10)
		to_chat(src, "<font color='#800080'>You need more biomass!</font>")
		return
	else
		...
		new /mob/living/simple_animal/hostile/rippers(src.loc)
		src.biomass -= 10
		visible_message("<span class='warning'>A few rippers burst from the ground and immediately begin to swarm!</span>")

/mob/living/carbon/human/genestealer/proc/neurotoxin(mob/target as mob in oview())
	set name = "Spit Neurotoxin (5)"
	set desc = "Spits neurotoxin at someone, minorly injuring their organs, blinding them for a bit and giving them some of dat good ol pain."
	set category = "Tyranid"

	if(src.biomass < 5)
		to_chat(src, "<font color='#800080'>You need more biomass to make neurotoxins!</font>")
		return
	...
	var/obj/item/projectile/energy/neurotoxin/genestealer/A = new /obj/item/projectile/energy/neurotoxin/genestealer(usr.loc)
	A.launch_projectile(target,get_organ_target())
	src.biomass -=5

/obj/item/projectile/energy/neurotoxin/genestealer
	name = "neuro"
	icon_state = "neurotoxin"
	damage = 10 //nonlethal but still damages your organs
	damage_type = TOX
	agony = 50 //painful but not a instastun
	eyeblur = 10 // *blurs your eyes* and walks away like a chad
...
/obj/item/stack/teeth/genestealer
	name = "weird teeth"
	singular_name = "weird tooth"
	icon = 'code/modules/economy/teef.dm'
	icon_state = "teef0" //ork teeth sprite
	drop_sound = null

//DO NOT REMOVE THESE 3 LINES
/obj/item/stack/teeth/genestealer/New()
	..()
	icon_state = "teef0"
...
//generation 1, maelignaci, look like nids but a bit mutated, really bad in social interactions just like a introvert
/datum/species/xenos/tyranids/Maelignaci
	name = SPECIES_GEN1TYRANID
	name_plural = "Maelignaci"
	icobase = 'icons/mob/human_races/tyranids/r_tyranid.dmi'
	deform = 'icons/mob/human_races/tyranids/r_def_tyranid.dmi'
	damage_mask = 'icons/mob/human_races/masks/dam_mask_human.dmi'
	blood_mask = 'icons/mob/human_races/masks/blood_human.dmi'
	...
	slowdown = -0.3
	unarmed_types = list(
		/datum/unarmed_attack/rendingclaws/weaker,
		)

	has_fine_manipulation = 1
	siemens_coefficient = 1
	darksight = 16 //only works until 8 but idc
	brute_mod = 0.9 // Hardened carapace.
	burn_mod = 1.4 // Hardened carapace.
	blood_color = "#8c0760" //purple ichor or so

//generation 2, Hybrids, look like mutants, can speak low gothic.
/datum/species/xenos/tyranids/Hybrid
	name = SPECIES_GEN2TYRANID
	name_plural = "Hybrids"
	...
	min_age = 16
	max_age = 40 //can't be old enough to be useless for the hive, otherwise he wouldn't BE in this bloody revolution!
	total_health = 250
	mob_size = MOB_NORMAL
	strength = STR_HIGH //stronger than average humans
	...
	slowdown = -0.3
	unarmed_types = list(
		/datum/unarmed_attack/rendingclaws/minor,
		)

	has_fine_manipulation = 1
	siemens_coefficient = 1
	darksight = 0 //no darkvision, not very lore accurate, but trust me, its better this way
	brute_mod = 0.9 // Hardened carapace.
	burn_mod = 1.1 // Hardened carapace.

//Generation 3 and 4, True Hybrids, can speak low and high gothic.
/datum/species/xenos/tyranids/TrueHybrid
	name = SPECIES_GEN3TYRANID
	name_plural = "Hybrids"
	icobase = 'icons/mob/human_races/r_human.dmi'
	deform = 'icons/mob/human_races/r_def_human.dmi'
	damage_mask = 'icons/mob/human_races/masks/dam_mask_human.dmi'
